...
async function start() {
  // fetch live data
  const res = await fetch("http://localhost:4000/api/stories/graph");
  const json = await res.json();

  // build graph from fetched JSON
  const graph = populateGraph(json);
  ...
  function populateGraph(json) {
    ...
    const g = createGraph();
    json.nodes.forEach((n) => g.addNode(n.id, n.data));
    json.links.forEach((l) => g.addLink(l.source, l.target));
    return g;
  }
